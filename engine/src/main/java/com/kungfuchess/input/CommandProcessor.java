...
public final class CommandProcessor {
    ...
    public void process(String line) {
        ...
        switch (parts[0].toLowerCase()) {
            case CommandSyntax.COMMAND_CLICK:
                handleClick(parts);
                break;
            case CommandSyntax.COMMAND_JUMP:
                handleJump(parts);
                break;
            case CommandSyntax.COMMAND_WAIT:
                handleWait(parts);
                break;
            case CommandSyntax.COMMAND_PRINT:
                handlePrint(parts);
                break;
            default:
                break;
    ...
    private void handlePrint(String[] parts) {
        if (parts.length >= 2 && parts[1].equalsIgnoreCase(CommandSyntax.PRINT_ARG_BOARD)) {
